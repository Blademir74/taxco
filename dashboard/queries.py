...
def get_mapa_ganadores(anio):
    ...
    ganador_por_seccion AS (
        SELECT DISTINCT ON (pk_seccion) pk_seccion, seccion, partido as ganador, votos as votos_ganador
...
def get_perfil_genero():
    query = """
    SELECT s.seccion, pi.lista_nominal_oficial, pi.lista_mujeres, pi.lista_hombres,
...
def get_correlacion_participacion_carencias(anio):
    ...
    SELECT ps.seccion, COALESCE(ps.participacion_pct, 0) as participacion_pct,
        ci.pct_sin_derechohab, ci.grado_prom_escolar,
        ROUND((COALESCE(ci.vph_sin_agua::numeric, 0) * 100.0 / NULLIF(ci.num_viviendas_particulares, 0) +
    ...
    WHERE ci.pobtot > 0 AND ci.anio_inegi = (SELECT MAX(anio_inegi) FROM carencias_inegi)
    ORDER BY ps.seccion
    """
...
def get_riesgo_electoral():
    query = """
    SELECT seccion, ganador_2024, votos_ganador, lista_nominal_oficial, pct_votos,
        indice_satisfaccion, num_opiniones, nivel_riesgo_electoral, accion_recomendada
...
# ============================================
# 14. ACCIONES PRIORITARIAS 24H (CORREGIDA)
# ============================================
def get_acciones_prioritarias_24h(top_n=3):
    query = """
    WITH top20 AS (
        SELECT pi.pk_seccion, s.seccion, pi.lista_nominal_oficial,
